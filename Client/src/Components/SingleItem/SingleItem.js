...
export default function SingleItem() {
    const { isLoading, setIsLoading, addItem } = useGlobalContext()
    ...
    const getSingleProduct = async (itemID) => {
        if (itemID) {
            setIsLoading(true)
            await fetch(`https://fakestoreapi.com/products/${itemID}`).then(resp => resp.json()).then(data => setSingleProduct(data)).catch(err => console.log("Can't fetch", err))
            setIsLoading(false)
        }
    ...
    return (
        <div className="single_product_container" key={itemID}>
            ...
            <div className="right_side">
                ...
                <div className="cost">
                    <span>₹{price}</span>
                    <span>₹{Math.floor((price + 30 / 100 * price))}</span>
                    <span>30% Off</span>
                ...
                <div className="delivery">
                    ...
                    <div className="delivery_options">
                        <svg width="13" height="12">
                            <path fill="#2874f0" className="_6xm1dD" d="M4.2 5.7c-.828 0-1.5-.672-1.5-1.5 0-.398.158-.78.44-1.06.28-.282.662-.44 1.06-.44.828 0 1.5.672 1.5 1.5 0 .398-.158.78-.44 1.06-.28.282-.662.44-1.06.44zm0-5.7C1.88 0 0 1.88 0 4.2 0 7.35 4.2 12 4.2 12s4.2-4.65 4.2-7.8C8.4 1.88 6.52 0 4.2 0z" fillRule="evenodd"></path></svg>
